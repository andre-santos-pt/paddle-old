...
public class Paddle2Java implements IModuleTranslator {
	...
	public String translate(IModule m) {
		...
		String code = "class " + m.getId() + " {\n";

		//		IRecordType mainT = m.getRecordType(modId);
		...
		for(String ns : m.getNamespaces()) {
			...
			if(type != null) {
				...
				for (IVariableDeclaration member : type.getFields()) {
					code += "\t" + type(member.getType()) + " " + member.getId() + ";\n";
				}
		...
		//		for(IRecordType r : m.getRecordTypes())
		////			if(!modId.equals(r.getId()))
		//				code += declaration(r);

		return code + "}\n";
	}

	String argsToString(List<IExpression> arguments) {
		String args = "";
	...
	public String declaration(IRecordType t) {
		...
		for (IVariableDeclaration member : t.getFields()) {
			text += "\t" + type(member.getType()) + " " + member.getId() + ";\n";
		}
		text += "}\n";
		return text;
	}
	...
	private String header(IProcedure p) {
		String text = Keyword.STATIC + " " + type(p.getReturnType()) + " " + p.getId() + "(";
		String args = "";
	...
	public String translate(IBlockElement e) {
		...
		if(e instanceof IVariableDeclaration) {
			...
			ret = a.getTarget().getId() + "." + a.getField().getId() + " = " + translate(a.getExpression()) + ";";
		}
...
//		String id = call.getOwnerProcedure().getNamespace().equals(call.getProcedure().getNamespace()) ?
//				call.getProcedure().getId() :
		String id =	call.getProcedure().getNamespace() + "." + call.getProcedure().getId();
		return id + "(" + argsToString(call.getArguments()) + ")";
	}
